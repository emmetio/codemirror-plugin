...
var consumeColor = function(stream) {
	...
	if (stream.peek() === HASH$1) {
		...
		stream.start = stream.pos;
		if (stream.eat(DOT$2) && !stream.eatWhile(isNumber$1)) {
...
function eatNumber(stream) {
	...
	while (!stream.eof()) {
		...
		if (code === DOT$1$1 ? hadDot : !isNumber$1(code)) {
			break;
		}
...
var prototypeAccessors$3 = { defaulValue: { configurable: true } };
 ...
 * @return {Node}
 */
function parse$1$1(abbr, options) {
	return index(abbr)
 ...
 * @return {Node}
 */
function parse$2$1(abbr, options) {
	if (typeof abbr === 'string') {
		abbr = index$4(abbr);
	}
...
function parse$$1(abbr, options) {
	...
	return isStylesheet(options.syntax)
		? parse$2$1(abbr, options)
		: parse$1$1(abbr, options);
}
...
function narrowToNonSpace(editor, from, to) {
	var stream = new CodeMirrorStreamReader(editor, from);
 ...
 */
function extractAbbreviation$1(editor, pos) {
	pos = pos || pos.getCursor();
	var line = editor.getLine(pos.line);

	return extractAbbreviation(line, pos.ch, true);
}
 ...
 */
function expandAndInsert(editor, abbr, range, options) {
...
function markAbbreviation(editor, pos, forced) {
	...
	var extracted = extractAbbreviation$1(editor, pos);
 ...
 * @param  {CodeMirror} editor
 */
function emmetExpandAbbreviation(editor) {
	...
	if (marker) {
		...
		var abbrData = extractAbbreviation$1(editor, pos);
		if (abbrData) {
			var range = {
				from: {
					line: pos.line,
...
function emmetInsertLineBreak(editor) {
	var between = editor.listSelections().map(function (sel) { return betweenTags(editor, sel); });
	...
	editor.operation(function () {
		...
		var singleSep = editor.doc.lineSeparator();
		var doubleSep = singleSep + singleSep;
		...
		for (var i$1 = 0; i$1 < sels.length; i$1++) {
			editor.indentLine(sels[i$1].from().line, null, true);

			if (between[i$1]) {
				editor.indentLine(sels[i$1].from().line - 1, null, true);
			}
		}

		// Step 3: adjust caret positions
		editor.setSelections(editor.listSelections().map(function (sel, i) {
			if (between[i]) {
				var line = sel.from().line - 1;
				var cursor = {
					line: line,
...
function wrapWithAbbreviation(editor) {
	var range = getWrappingContentRange(editor);

	if (range) {
		var prompt = editor.getOption('emmetPrompt') || defaultPrompt;
		var text = editor.getRange(range.from, range.to, '\n')
			.split('\n')
			.map(function (line) { return line.trim(); });

		prompt(editor, 'Enter abbreviation to wrap with:', function (abbr) {
			if (abbr) {
				expandAndInsert(editor, abbr, range, { text: text });
			}
		});
	} else {
...
function getWrappingContentRange(editor) {
	if (editor.somethingSelected()) {
		var sel = editor.listSelections().filter(function (sel) { return sel.anchor !== sel.head; })[0];
		if (sel) {
...
function getTagRangeForPos(editor, pos) {
	...
	var tag = model && model.nodeForPoint(pos);
	...
	// Inner range
	var from = tag.open.end;
	var to = tag.close ? tag.close.start : tag.open.end;
 ...
 */
function autocompleteProvider(editor, abbrModel, abbrPos, cursorPos) {
...
var prototypeAccessors$4 = { name: { configurable: true },attributes: { configurable: true },start: { configurable: true },end: { configurable: true },firstChild: { configurable: true },nextSibling: { configurable: true },previousSibling: { configurable: true } };
...
var eatPaired = function(stream) {
	...
	var consumed = eatPair(stream, LANGLE, RANGLE, opt$2)
		...
		|| eatPair(stream, LCURLY,  RCURLY,  opt$2);
...
function isTagName(code) {
	return isAlphaNumeric(code)
		...
		|| code === DOT$3   // in rare cases declarative tag names may have dots in names
		|| code === DASH$5
		|| code === UNDERSCORE;
}
	...
	while (!stream.eof()) {
		if (m = match(stream)) {
			...
			if (m.type === 'open') {
				...
				last(stack).addChild(new Node$2(stream, m.type, m));
			}
		} else {
			stream.next();
		}
...
var commands = { emmetExpandAbbreviation: emmetExpandAbbreviation, emmetInsertLineBreak: emmetInsertLineBreak, emmetWrapWithAbbreviation: wrapWithAbbreviation };
