...
export default function(editor, pos) {
	...
	if (abbreviation) {
		const completions = abbreviation.config.type === 'stylesheet'
			? getStylesheetCompletions(editor, abbreviation, pos)
			: getMarkupCompletions(editor, abbreviation, pos);
 ...
 * @return {EmmetCompletion[]}
 */
export function getMarkupCompletions(editor, abbrModel, pos) {
	let result = [];
	pos = pos || editor.getCursor();
	...
	// class, id etc.)
	const prefix = getMarkupPrefix(abbrModel.abbreviation, pos.ch - abbrModel.pos.ch);
	if (prefix !== null) {
		const prefixRange = {
		...
		const completions = getSnippetCompletions(editor, pos, abbrModel)
 ...
 * @return {EmmetCompletion[]}
 */
function getStylesheetCompletions(editor, abbrModel, pos) {
	let result = [];
	pos = pos || editor.getCursor();
	...
	// Make sure that current position precedes element name (e.g. not attribute,
	// class, id etc.)
	const prefix = getStylesheetPrefix(abbrModel.abbreviation, pos.ch - abbrModel.pos.ch);
	if (prefix !== null) {
		const prefixRange = {
			from: { line: pos.line, ch: pos.ch - prefix.length },
			to: pos
