		...
		if (isCSSPropertyValue(editor, pos)) {
			...
			if (prop) {
				...
				const completion = getSnippetCompletions(editor, pos, config)
					.find(item => item.snippet && item.snippet.property === lowerProp);
	
				if (completion && completion.keywords.length) {
					return completion.keywords.map(kw => {
						return kw.key.indexOf(prefix) === 0 && new EmmetCompletion('value', editor, prefixRange, kw.key, kw.value, kw.value);
					}).filter(Boolean);
				}
			}
		} else {
